...
func (s *server) handlerShortenLink(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		httpError(r.Context(), w, http.StatusInternalServerError, fmt.Errorf("failed ot shorten url: %w", err))
...
func (s *server) handlerRedirect(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if errors.Is(err, store.ErrNotFound) {
			...
			httpError(r.Context(), w, http.StatusNotFound, fmt.Errorf("internal server error: %w", err))
		}
		return
	}
