...
func Test_requestLogger(t *testing.T) {
	...
	if logBuffer.String() != expectedLogString {
		t.Errorf("EXPECTED : %v, ACTUAL : %v", expectedLogString, logBuffer.String())
	}
	if rr.Code != http.StatusOK {
		t.Errorf("status code %v does not match expected status code %v", rr.Code, expectedStatusCode)
	}
}
